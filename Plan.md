...
1. **Workout notes / RPE per set.** Optional field on `CompletedSet` ("felt
   heavy," RPE 8) — turns the log into a training journal.
2. **Auto-progression suggestions surfaced in-UI.** `progression-service.ts`
...
3. **Apple Watch companion (iOS side).** WatchKit extension showing current
...
4. **Program templates / sharing.** "Duplicate as new program" plus
...
7. **Loop band resistance ranges.** Serious Steel bands show a known lb
...
8. **Loop band `Equip Detail` validation.** Band colors must match the known
...
**#1 — workout notes / RPE per set.** Cheapest way to turn the log into a
training journal before layering on more progression UI.
