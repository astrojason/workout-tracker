...
describe("parseCSV - error handling", () => {
  ...
  it("accepts dumbbell as a valid equipment type", () => {
    const csv = `${VALID_HEADER}\n${makeRow({ equipment_type: "dumbbell" })}`;
  ...
  it("accepts loop_band as a valid equipment type", () => {
    const csv = `${VALID_HEADER}\n${makeRow({ equipment_type: "loop_band" })}`;
    expect(() => parseCSV(csv)).not.toThrow();
    expect(parseCSV(csv).workouts[0].exercises[0].equipmentType).toBe("loop_band");
  });
